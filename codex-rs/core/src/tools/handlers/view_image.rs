...
mod tests {
    ...
    async fn pre_tool_use_payload_emits_parsed_arguments() {
        let arguments = json!({ "path": "/tmp/example.png", "detail": "original" });
        ...
        assert_eq!(
            ViewImageHandler.pre_tool_use_payload(&invocation),
            Some(PreToolUsePayload {
    ...
    async fn pre_tool_use_payload_skips_non_function_payloads() {
        ...
        assert_eq!(ViewImageHandler.pre_tool_use_payload(&invocation), None);
    }
}
